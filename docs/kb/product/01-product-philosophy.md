...
**Aktueller Implementierungsstand (2026-08-19):** Links im Header steht `Letzte 7 Tage`; rechts oben steht einzeilig `Zielerreichung: <Wert>`. Darunter folgt rechtsbündig nur `<Gegessen> / <Ziel> kcal` ohne sichtbares Ziel-Label. Die beiden unteren Bilanzzeilen, einschließlich `Ø Ziel / Tag`, sind entfernt. Über den Balken stehen die Tages-kcal ohne wiederholte Einheit; Accessibility-Beschreibungen nennen weiterhin die vollständigen kcal-Werte. Diese Regel ersetzt die frühere Beschreibung der unteren Bilanzzeilen.
